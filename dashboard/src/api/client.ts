...
export async function getLatestAttestation(
  ...
  return fetchJson(`${API_BASE}/attestations/latest/${orgIdHash}`);
}

export async function verifyChain(
  orgIdHash: string
): Promise<VerifyChainResponse> {
  return fetchJson(`${API_BASE}/attestations/verify/${orgIdHash}`);
}

export async function submitAttestation(
  body: Record<string, unknown>
): Promise<AttestationSubmitResponse> {
  return mutateJson(`${API_BASE}/attestations/submit`, {
    method: "POST",
    body: JSON.stringify(body),
  });
}
