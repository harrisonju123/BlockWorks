...
from agentproof.alerts.types import AlertChannel, AlertEvent, AlertRule, AlertSeverity
...
def format_email_body(
    ...
    *,
    ...
    lines = [
        f"AgentProof Alert: {event.severity.value.upper()}",
    ...
    lines.extend(["", "---", "View details in the AgentProof dashboard."])
...
async def send_email(
    *,
    ...
    smtp_host: str,
    smtp_port: int = 587,
    smtp_from: str = "alerts@agentproof.io",
) -> bool:
    ...
    *,
    ...
    if rule.channel in (AlertChannel.EMAIL, AlertChannel.BOTH):
        ...
        if email_to and smtp_host:
            ...
            await send_email(
                to=email_to,
                subject=f"[AgentProof] {rule.rule_type.value} alert - {event.severity.value}",
                body=body,
                smtp_host=smtp_host,
                smtp_port=smtp_port,
                smtp_from=smtp_from,
