"""Async and sync HTTP clients for the BlockThrough API.

BlockThroughSDK is the primary async client using httpx.AsyncClient.
BlockThroughClient is a sync wrapper that creates a one-shot async
...
class BlockThroughError(Exception):
    """Raised when the BlockThrough API returns a non-2xx response."""

    def __init__(self, status_code: int, detail: str) -> None:
        self.status_code = status_code
        self.detail = detail
        super().__init__(f"BlockThrough API error {status_code}: {detail}")


class BlockThroughSDK:
    """Async Python SDK for BlockThrough.

    Connects to the BlockThrough API via httpx.AsyncClient. Handles
    ...
    async def __aenter__(self) -> BlockThroughSDK:
        ...
        *,
        ...
        if response.status_code >= 400:
            ...
            try:
                ...
                pass
            raise BlockThroughError(response.status_code, str(detail))
    ...
    async def track(
        ...
        *,
        ...
        """Report a single LLM call to BlockThrough.
...
class BlockThroughClient:
    """Synchronous wrapper around BlockThroughSDK.
    ...
    applications, use BlockThroughSDK directly.
    """
    ...
    def __enter__(self) -> BlockThroughClient:
        self._ensure_client()
        return self
    ...
    def track(
        ...
        *,
        ...
        """Report a single LLM call to BlockThrough (synchronous)."""
        event_id = str(uuid.uuid4())
        now = utcnow().isoformat()
...
AgentProofError = BlockThroughError
AgentProofSDK = BlockThroughSDK
AgentProofClient = BlockThroughClient
