...
from blockthrough import __version__
from blockthrough.api.routes import (
    alerts,
    attestation,
    benchmarks,
    channels,
...
from blockthrough.benchmarking.mirror import BenchmarkWorker
from blockthrough.benchmarking.types import BenchmarkConfig
from blockthrough.config import get_config
from blockthrough.db.queries import get_active_routing_policy, get_fitness_matrix
from blockthrough.pipeline.writer import EventWriter
from blockthrough.routing.policy import PolicyValidationError, default_policy, load_policy
from blockthrough.routing.router import FitnessCache, generate_synthetic_fitness, merge_fitness_entries
from blockthrough.routing.writer import DecisionRecord, RoutingDecisionWriter
    ...
    while True:
        try:
            ...
            if cfg.benchmark_smart_targeting and bench_config is not None:
                ...
                from blockthrough.db.queries import get_waste_analysis
...
async def lifespan(app: FastAPI):
    ...
    if cfg.routing_enabled:
        ...
        try:
            from blockthrough.api.deps import get_async_session
            from blockthrough.api.routes.routing import _load_policy_from_row
            async with get_async_session() as session:
        ...
        app.state.upstream_models = None
        for attempt in range(5):
            try:
                resp = await anthropic_client.get("/v1/models")
                if resp.status_code == 200:
                    upstream_models = {
                        m["id"] for m in resp.json().get("data", []) if isinstance(m, dict)
                    }
                    app.state.upstream_models = upstream_models
                    logger.info("Fetched %d upstream models for routing filter", len(upstream_models))
                    break
                logger.warning("Failed to fetch upstream models (status=%d, attempt %d)", resp.status_code, attempt + 1)
            except Exception:
    ...
    if cfg.attestation_scheduler_enabled:
        from blockthrough.api.routes.attestation import _get_provider
        from blockthrough.attestation.scheduler import run_attestation_scheduler
